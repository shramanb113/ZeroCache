...
5. **End-to-end, consumer 2**: real batch run through a second, Python-based ingestion script (LangChain/LlamaIndex) — this is the test that actually validates the neutrality claim, not #4. **Still not started as originally scoped (Python).** What did happen (2026-07-24): `demo/langchain-ts/` — a real RAG pipeline (chunk → embed via Zerocache → `MemoryVectorStore` → similarity-search query) built with `@langchain/openai`, plus a battle-test harness hammering the HTTP contract from an unmodified LangChain client's perspective (17 real-key tests + 11 key-independent ones, all passing against a live local instance with a real OpenAI key). This is genuine validation, but be precise about what it proves: LangChain and Mastra are both TypeScript/npm consumers, so this proves "a second, independently-built TS framework works," not the harder "works in a completely different language ecosystem" claim #15 in the PRD is actually after — that still needs a real Python consumer. What it *did* catch that mock-based testing structurally couldn't: `input` rejecting a bare string (Deviations item 9) — a real, load-bearing compatibility bug that broke `embedQuery()` (and therefore `similaritySearch()`'s query step, i.e. a RAG app's entire question-answering path) 100% of the time, invisible to every existing test because they all constructed `input` as an array by convention. Also produced one measured, non-theoretical number for the coalescing gap discussed in `decisions.md`/session history: 5 concurrent requests for one never-before-seen text produced 5 independent provider calls, not 1 — concrete evidence for a gap that was previously only described, not measured. **Both findings are now fixed** (2026-07-24, same day) — see Deviations items 10 and 11.
