...
- **Hybrid wait (pub/sub + poll).** Redis `PUBLISH`/`psubscribe` for a sub-millisecond wake, a 250 ms store re-read as the backstop for a missed publish (the SET-NX/subscribe race). Poll-only was the simpler fallback; the user chose the hybrid for wake latency. The wake is edge-triggered (a per-slot generation counter, snapshotted on entry to `follow`): a level flag is never cleared, so every later `follow` for that key would return instantly and the wait loop would become a full-rate poll. Missing an edge published between a caller's store re-read and its `follow` costs one poll interval, never a hang -- which is what the 250 ms backstop is for. The leader also writes the entry *before* signalling, so `done` means "readable", not just "the provider call returned".
...
- **Leader failure -> followers fall back to their own call.** No negative marker, no error caching -- preserves "never cache a non-2xx". The leader `complete`s (releases + signals) on *every* outcome, non-2xx and transport error alike, so peers stop waiting. A follower woken with the entry still absent breaks straight to the promotion attempt rather than polling out its 30 s deadline -- "the leader is done and stored nothing" is exactly when promoting is right. One follower promotes and does the call.
